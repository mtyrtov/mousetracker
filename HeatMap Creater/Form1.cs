...
namespace HeatMap_Creater
{
    public partial class Form1 : Form
    {
        ...
        private List<HeatPoint> readData()
        {
            List<Coords> records;

            using (var reader = new StreamReader(@"C:\Users\m_tyrtov\Desktop\1594064427.csv"))
            using (var csv = new CsvReader(reader, CultureInfo.InvariantCulture))
            {
                records = csv.GetRecords<Coords>().ToList();
            }

            int[,] maps = new int[1920, 1080];

            foreach (var item in records)
            {
                maps[item.x, item.y] += 1;
            }
            ...
            for (int i = 0; i < 1920; i++)
            {
                for (int j = 0; j < 1080; j++)
                {
                    if(maps[i,j] > 1)
                    {
                        result.Add(new HeatPoint(i, j, Convert.ToByte(maps[i, j] * 1)));
                    }
                }
            }
        ...
        private Bitmap CreateIntensityMask(Bitmap bSurface, List<HeatPoint> aHeatPoints)
        {
            ...
            foreach (HeatPoint DataPoint in aHeatPoints)
            {
                ...
                DrawHeatPoint(DrawSurface, DataPoint, 75);
            }
            return bSurface;
        }
